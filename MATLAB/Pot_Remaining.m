...
% This code is to generate potential energies for the scenario where
% individual cell types produce bistable and monostable responses and the PsB
...
sig1 = [1,7,14,80,102]; % B,T,M = Tristable, S = Bis. PsB = Tristable

sig2 = [0.8,6,12,80,98.8];% All Bistable, PsB Tristable

sig3 = [2,20,36,42,100]; % All monostable, PsB Tristable
...
u1=[0:delb:2500];     % range for argument in potential function
...
plot(u1,z1)
hold on
plot(u1(TF_max),z1(TF_max),'ro','MarkerFaceColor','r')
hold on

plot(u1(TF_min),z1(TF_min),'blacko','MarkerFaceColor','black')
hold on
